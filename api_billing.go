...
type ApiUpdateOverageSettingsRequest struct {
	ctx context.Context
	ApiService *BillingAPIService
	billingOverageRequest *BillingOverageRequest
}

func (r ApiUpdateOverageSettingsRequest) BillingOverageRequest(billingOverageRequest BillingOverageRequest) ApiUpdateOverageSettingsRequest {
	r.billingOverageRequest = &billingOverageRequest
	return r
}

func (r ApiUpdateOverageSettingsRequest) Execute() (*BillingOverageResponse, *http.Response, error) {
	return r.ApiService.UpdateOverageSettingsExecute(r)
}

/*
UpdateOverageSettings Update Overage Settings

Turn overage billing on or off for this account.

Off by default and stays off until asked: past the quota the API returns
429, which is a limit the customer can see coming. Overage replaces that
limit with a charge, and nobody should meet that decision on an invoice.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @return ApiUpdateOverageSettingsRequest
*/
func (a *BillingAPIService) UpdateOverageSettings(ctx context.Context) ApiUpdateOverageSettingsRequest {
	return ApiUpdateOverageSettingsRequest{
		ApiService: a,
		ctx: ctx,
	}
...
func (a *BillingAPIService) UpdateOverageSettingsExecute(r ApiUpdateOverageSettingsRequest) (*BillingOverageResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPatch
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *BillingOverageResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "BillingAPIService.UpdateOverageSettings")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v1/billing/overage"

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.billingOverageRequest == nil {
		return localVarReturnValue, nil, reportError("billingOverageRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.billingOverageRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
